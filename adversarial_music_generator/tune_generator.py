...
class TuneGenerator(TuneGeneratorInterface):
    ...
    def _generate_one_track(self, seed: Seed, track_number: int) -> Track:
        notes: List[Note] = []
        for i in range(0, seed.randint(1, 10, f'number of notes for track {track_number}')):
            ...
            note = Note(start_time_seconds=start_time, end_time_seconds=start_time + length, pitch=pitch)
            notes.append(note)
